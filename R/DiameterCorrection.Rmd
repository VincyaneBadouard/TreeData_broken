...
if (any(!is.na(POM))) { # POM exists?
  ...
  for (rs in 1:length(raised)){ # autant de rs que de changements de POM
    ...
    DBHCor_Vin[raised[rs]:min(raised[rs+1]-1, length(DBHCor_Vin), na.rm = TRUE)] <-
      ...
      mean(cresc[((raised[rs]+1):min(raised[rs+1]-1, length(DBHCor_Vin), na.rm = TRUE))-1]) *(Time[raised[rs]+1]-Time[raised[rs]]) # mean of the annual diameter increments in the shift, multiplied by the number of years between the census of the shift and the previous one.
...
if (Ncresc_abn > 0) { # if there are abnormal values
  for (i in 1:Ncresc_abn) { # for each abnormal value
    ...
    ab <- cresc_abn[which.max(abs(cresc[cresc_abn]))] # the highest absolute DBH increment (celui qui a initié)
    ...
    if (length(ab) == 1) {
      ...
      meancresc = max(mean(cresc[surround], na.rm = TRUE), 0) # mean(cresc) of these 4 values. 0 is better than a negative cresc
