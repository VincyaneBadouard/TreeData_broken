...
- Check *missing value* in X-YTreeUTM/PlotArea/Plot/SubPlot/Year/TreeFieldNum/
...
for (s in unique(na.omit(Data$Site))) {
  ...
  CoordIDCombination <- na.omit(unique(
    Data[Data$Site == s, .(IdTree, XTreeUTM, YTreeUTM)]
  ))
  ...
  if(!identical(CorresIDs, unique(CorresIDs))){ # check if it's the same length, same ids -> 1 asso/ID
    ...
    Data <- GenerateComment(Data,
                            condition =
                              Data[,Site] == s
                            & Data[,IdTree] %in% duplicated_ID,
                            comment = "Different coordinates (XTreeUTM, YTreeUTM) for a same IdTree")
  }
} # end site loop
