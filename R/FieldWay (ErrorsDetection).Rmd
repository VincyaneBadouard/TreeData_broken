...
Data[, PlotSubNum := paste(CensusYear, Plot, SubPlot, TreeFieldNum, sep = "/")]
  ...
  for (y in unique(na.omit(Data$CensusYear))) {
    ...
    for (p in unique(na.omit(Data$Plot))) {
      ...
      for (c in unique(na.omit(Data[Data$Plot==p, "SubPlot"]))) {
        ...
        num <- Data[Data$CensusYear == y & Data$Plot == p & Data$SubPlot == c,]$TreeFieldNum # all the TreeFieldNum for each Plot-SubPlot combination
        ...
        if(anyDuplicated(num) != 0){
          ...
          Data[Data$CensusYear == y & Data$Plot == p & Data$SubPlot == c & Data$TreeFieldNum %in% duplicated_num,
