...
DataTree <- PanamaFormated[IdStem %in% "443106"]
Data <- PanamaFormated
DefaultHOM = 1.3
MaxDBH = 500
...
#   MinIndividualNbr = 1)
# 
# save(Rslt, file = "PanamaDBHCor.rda")
load("D:/VSC TmFO/Data/StandardisedData/PanamaDBHCor.rda")
```
...
pl <- lapply(IdStemCor, function(x)
  ggplot(Rslt[IdStem == x]) +
    aes(x = Year) +
    ...
    geom_point(aes(y = Diameter,
               ...
               shape = "circle", size = 3.9) +
    geom_text(aes(y = Diameter, label = HOM, colour = "HOM"), position = position_nudge(y = -0.3)) +
    geom_line(aes(y = Diameter, color = ifelse(Diameter != DBHCor, 'Initial', 'Conserved'))) +
    
    # Corrected
    geom_point(aes(y = DBHCor,
                   color = ifelse(Diameter != DBHCor, 'Corrected', 'Conserved')),
               shape = "circle", size = 3.9) +
    geom_text(aes(y = DBHCor, label = HOMCor, colour = "HOM"), position = position_nudge(y = -0.3)) +
    geom_line(aes(y = DBHCor, color = ifelse(Diameter != DBHCor, 'Corrected', 'Conserved'))) +
    ...
    labs(title =  paste("IdStem: ",unique(RsltCor$IdStem),""),
         x ="Year", y = "Diameter (cm)") #+
)

# ggforce::facet_wrap_paginate(vars(IdStem), scales = "free", ncol = 3, nrow = 3, page = 1) # 8 pages

ml <- gridExtra::marrangeGrob(pl, nrow=3, ncol=3)
...
pdf("DiameterCorrectionPlots2.pdf", width = 25, height = 10)
  ...
  print(ggplot(RsltCor) +
          ...
          geom_point(data = subset(RsltCor, !is.na(Diameter)),
                     aes(y = Diameter,
                         color = ifelse(Diameter != DBHCor, 'Initial', 'Conserved')),
                     shape = "circle", size = 3.9) +
          geom_text(data = subset(RsltCor, (!is.na(Diameter) & !is.na(HOM))),
          ...
          geom_point(data = subset(RsltCor, !is.na(DBHCor)),
                     aes(y = DBHCor,
                         color = ifelse(Diameter != DBHCor, 'Corrected', 'Conserved')),
                     shape = "circle", size = 3.9) +
          geom_text(data = subset(RsltCor, (!is.na(DBHCor) & !is.na(HOMCor))),
