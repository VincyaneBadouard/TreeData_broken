...
DiameterCorrectionPlot(Rslt_taper, OnlyCorrected = TRUE, CorCol = "TaperCorDBH")
```
### Plot - Taper Correction
```{r}
Rslt <- Rslt_taper
...
IdStemCor <- Rslt[Diameter != TaperCorDBH, IdStem] #  corrected stems
...
for(p in 1:(ceiling(length(unique(IdStemCor))/9))){ 
  print(ggplot(RsltCor) +
          ...
          #            aes(y = Diameter,
          #                color = if(is.na(TaperCorDBH)) 'Duplicated measurement'),
          #            shape = "circle", size = 3.9) +
          # Initial 
          geom_point(data = subset(RsltCor, !is.na(Diameter)),
                     aes(y = Diameter,
                         color = ifelse(Diameter != TaperCorDBH, 'Initial', 'Conserved')),
                     shape = "circle", size = 3.9) +
          geom_line(data = subset(RsltCor, !is.na(Diameter)),
                    aes(y = Diameter, color = ifelse(Diameter != TaperCorDBH, 'Initial', 'Conserved'))) +
          ...
          # Corrected 
          geom_line(data = subset(RsltCor, !is.na(TaperCorDBH)),
                    aes(y = TaperCorDBH, color = ifelse(Diameter != TaperCorDBH, 'Corrected', 'Conserved'))) +
          
          geom_point(data = subset(RsltCor, !is.na(TaperCorDBH)),
                     aes(y = TaperCorDBH,
                         color = ifelse(Diameter != TaperCorDBH | is.na(Diameter), 'Corrected', 'Conserved')),
                     shape = "circle", size = 3.9) +
          
          geom_text_repel(data = subset(RsltCor,
                                        (!is.na(TaperCorDBH) & !is.na(HOMCor) & (Diameter != TaperCorDBH) | is.na(Diameter))),
                          aes(y = TaperCorDBH, label = HOMCor, colour = "HOM"),
                          point.size = 3.9, size = 3, direction = "y") +
          geom_text_repel(data = subset(RsltCor, (!is.na(TaperCorDBH) & DiameterCorrectionMeth != "")),
                          aes(y = TaperCorDBH, label = DiameterCorrectionMeth, colour = "Methode"),
          ...
          # Titles 
          labs(
            # title =  paste("Scientific name: ",unique(RsltCor$ScientificName),""),
            x ="Year", y = "Diameter (cm)") +
          
          
          ggforce::facet_wrap_paginate(vars(IdStem, ScientificName),
                                       scales = "free", ncol = 3, nrow = 3, page = p) 
...
RsltCor <- Rslt
IdStemCor <- Rslt[, IdStem] #  corrected stems

# IdStemCor <- Rslt[Diameter != DBHCor, IdStem] #  corrected stems
# 
# RsltCor <- Rslt[IdStem %in% IdStemCor] #  corrected stems

pdf("DiameterCorrectionPlots_Ind_all.pdf", width = 25, height = 10)
for(p in 1:(ceiling(length(unique(IdStemCor))/9))){ 
  print(ggplot(RsltCor) +
          aes(x = Year) +
          # Duplicated measurement
          ...
          ggforce::facet_wrap_paginate(vars(IdStem, ScientificName), scales = "free", ncol = 3, nrow = 3, page = p) 
