...
Remove : !"#$%&’()*+,-./:;<=>?@[]^_`{|}~
```{r}
Data$ScientificNameCor <- gsub("[[:punct:]]", "", Data$ScientificName)

# Comment : "Special characters in the 'ScientificName'"
...
RsltSfc <- Taxonstand::TPL(unique(Data$ScientificName), corr = TRUE, diffchar = 20, max.distance = 1) # diffchar: maximum difference of characters nbr between input and output

## with Genus and species (23 var)
RsltGS <- Taxonstand::TPL(genus = unique(Data$Genus), species = unique(Data$Species), infra = FALSE, corr = TRUE, diffchar = 20, max.distance = 1) # diffchar: maximum difference of characters nbr between input and output
...
# Take only Taxonomic.status == "Accepted" | "Synonym"

# Method : The Plant List

# Comment:
## if "Synonym" : 'ScientificName' is a synonym of the accepted botanical name
## if Typo == TRUE : Orthographic error in the 'ScientificName'
```
...
FamilyData <- 
  setDT( # as data.table
    BIOMASS::getTaxonomy(unique(Data$Genus), findOrder = FALSE)
  )

FamilyData <- setnames(FamilyData, "family", "FamilyCor") # rename columns


Data <- merge(Data, FamilyData, by.x = "Genus", by.y = "inputGenus", sort = FALSE)
