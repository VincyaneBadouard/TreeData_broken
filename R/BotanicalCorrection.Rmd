...
# Genus column with Family name
```{r}
# de aceae à un special character en lecture à l'envers
Data[, c("GenusCor", "GenspFamily") := tstrsplit(Genus, "[[:punct:]]", fixed = TRUE)]

sub.pattern = c(" sp[.]", " spp[.]", " pl[.]",
                "Indet[.]", " indet[.]", " ind[.]", " gen[.]", " g[.]", " fam[.]",
                " nov[.]", " prox[.]", " cf[.]", " aff[.]", " s[.]s[.]",
                " s[.]l[.]", " p[.]p[.]", " p[.] p[.]", "[?]", " inc[.]",
...
for (i in 1:length(sub.pattern)) {
  Data[, ScientificName := gsub(pattern = sub.pattern[i],
                                replacement = "", x = Data[, ScientificName])]
}


for (i in 1:nrow(Data)) {
  species.string <- Data[i, ScientificName]
  if (grepl("[[:digit:]]", species.string) ==
      TRUE) {
    Data[i, "Number.detected"] <- as.logical(1)
    ...
    Data[i, ScientificName] <- species.new.string
  }
}
```

# SubSpecies column if necessary
```{r}
SubSpeciesSep = "_subsp."
Data[, c("SpeciesCor", "SubSpeciesCor") := tstrsplit(SpeciesCor, SubSpeciesSep, fixed = TRUE)]
```
...
Data[, GenusCor := gsub("[!#$%&’+,:;<=>?@^`]", "", Data$GenusCor)]
Data[, SpeciesCor := gsub("[!#$%&’+,:;<=>?@^`]", "", Data$SpeciesCor)]
...
Data <- GenerateComment(Data,
                        condition = grepl('[!#$%&’+,:;<=>?@^`]', Data$Genus), # TRUE if there are any special character
                        comment = "Special characters in the 'Genus'")

Data <- GenerateComment(Data,
                        condition = grepl('[!#$%&’+,:;<=>?@^`]', Data$Species), # TRUE if there are any special character
...
# save(WFO_Backbone, file = "WFO_Backbone.rda", compress = "xz")
...
WFmatch <- WorldFlora::WFO.match(spec.data = setDF(ScientificNames), WFO.data = WFO_Backbone,
                                 Fuzzy.force = FALSE,
                                 Fuzzy.shortest = TRUE,
                                 verbose = FALSE) # (très long)
...
Data <- Data[grepl("Indet", Data$ScientificName) |
               grepl("[0-9]", Data$ScientificName) |
               grepl("subsp[.]", Data$ScientificName), ] 

# Data[grepl("Indet", Data$ScientificName, ignore.case = TRUE), ] 
# Data[grepl("sp[.]", Data$ScientificName), ]
# Data[grepl("subsp[.]", Data$ScientificName), ]
