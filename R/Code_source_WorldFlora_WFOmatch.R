# Code source de WorldFlora::WFO.match

function (spec.data = NULL, WFO.file = NULL, WFO.data = NULL,
          ...
          Fuzzy.force = FALSE, Fuzzy.max = 250, Fuzzy.min = TRUE,
          Fuzzy.shortest = FALSE, Fuzzy.within = FALSE, Fuzzy.two = TRUE,
          Fuzzy.one = TRUE, squish = TRUE, spec.name.tolower = FALSE,
          spec.name.nonumber = TRUE, spec.name.nobrackets = TRUE,
          exclude.infraspecific = FALSE,
          infraspecific.excluded = c("cultivar.",
                                     "f.", "sect.", "subf.", "subg.", "subsp.", "subvar.",
                                     "var", "var.", "[infraspec.]", "fo.", "forma", "nothosubsp.",
                                     "nothovar.", "sect."), spec.name.sub = TRUE,
          sub.pattern = c(" sp[.] A",
                          " sp[.] B", " sp[.] C", " sp[.]", " spp[.]", " pl[.]",
                          " indet[.]", " ind[.]", " gen[.]", " g[.]", " fam[.]",
                          " nov[.]", " prox[.]", " cf[.]", " aff[.]", " s[.]s[.]",
                          " s[.]l[.]", " p[.]p[.]", " p[.] p[.]", "[?]", " inc[.]",
                          " stet[.]", "Ca[.]", "nom[.] cons[.]", "nom[.] dub[.]",
                          " nom[.] err[.]", " nom[.] illeg[.]", " nom[.] inval[.]",
                          " nom[.] nov[.]", " nom[.] nud[.]", " nom[.] obl[.]",
                          " nom[.] prot[.]", " nom[.] rej[.]", " nom[.] supp[.]",
                          " sensu auct[.]"), verbose = TRUE, counter = 1000)
{
  if (class(spec.data) == "character") {
  ...
  if (is.null(WFO.data) == TRUE) {
    ...
    WFO.data <- data.table::data.table(WFO.data) # use data.table
  }
  ...
  if (spec.name %in% names(spec.data)) {
    ...
    if (spec.name.tolower == TRUE) {
      if (squish == FALSE) {
        spec.name.ORIG <- paste(spec.name, ".ORIG",
                                sep = "")
        spec.data[, spec.name.ORIG] <- spec.data[, spec.name]
      }
    ...
    if (spec.name.nonumber == TRUE) {
      ...
      spec.data$Number.detected <- rep(as.logical(0),
                                       nrow(spec.data))
      for (i in 1:nrow(spec.data)) {
        species.string <- spec.data[i, spec.name]
        if (grepl("[[:digit:]]", species.string) ==
            TRUE) {
  ...
  for (i in 1:nrow(spec.data)) {
    ...
    fuzzy.matches <- FALSE
    if (spec.name %in% names(spec.data) && nchar(spec.data[i,
                                                           spec.name]) > 0) {
      WFO.match <- WFO.data[WFO.data$scientificName ==
                              spec.data[i, spec.name], ]
      if (nrow(WFO.match) == 0) {
        if (spec.data[i, spec.name] == "Compositae") {
          WFO.match <- WFO.data[WFO.data$scientificName ==
                                  "Asteraceae", ]
        }
        if (spec.data[i, spec.name] == "Leguminosae") {
          WFO.match <- WFO.data[WFO.data$scientificName ==
                                  "Fabaceae", ]
        }
        if (spec.data[i, spec.name] == "Umbelliferae") {
          WFO.match <- WFO.data[WFO.data$scientificName ==
                                  "Apiaceae", ]
        }
        if (spec.data[i, spec.name] == "Palmae") {
          WFO.match <- WFO.data[WFO.data$scientificName ==
                                  "Arecaceae", ]
        }
        if (spec.data[i, spec.name] == "Cruciferae") {
          WFO.match <- WFO.data[WFO.data$scientificName ==
                                  "Brassicaceae", ]
        }
        if (spec.data[i, spec.name] == "Guttiferae") {
          WFO.match <- WFO.data[WFO.data$scientificName ==
                                  "Clusiaceae", ]
        }
        if (spec.data[i, spec.name] == "Labiatae") {
          WFO.match <- WFO.data[WFO.data$scientificName ==
                                  "Lamiaceae", ]
        }
        if (spec.data[i, spec.name] == "Gramineae") {
          WFO.match <- WFO.data[WFO.data$scientificName ==
                                  "Poaceae", ]
        }
      }
      if ((nrow(WFO.match) == 0 && Fuzzy > 0) || Fuzzy.force ==
          TRUE) {
        specFuzzy <- agrep(spec.data[i, spec.name],
                           x = WFO.data$scientificName, value = T, max.distance = Fuzzy)
        if (length(specFuzzy) == 0 && Fuzzy.two == TRUE) {
          ...
          if (length(species.terms) > 2) {
            species.string2 <- paste(species.terms[1],
                                     " ", species.terms[2], sep = "")
            specFuzzy <- agrep(species.string2, x = WFO.data$scientificName,
                               value = T, max.distance = Fuzzy)
            spec.data[i, "Fuzzy.two"] <- as.logical(1)
            if (verbose == TRUE && length(specFuzzy) >
                0) {
              message(paste("Fuzzy matches for ", spec.data[i,
                                                            spec.name], " were only found for first 2 terms",
                            sep = ""))
            }
          }
        }
            ...
            if ((nrow(WFO.match) == 0 && Fuzzy > 0) ||
              ...
              if (verbose == TRUE && length(specFuzzy) >
                ...
                message(paste("Fuzzy matches for ",
                              spec.data[i, spec.name], " were only found for first term",
                              sep = ""))
              }
            }
            else {
        ...
        if (length(specFuzzy) > Fuzzy.max) {
          ...
          if (verbose == TRUE) {
            message(paste("Too many (", length(specFuzzy),
                          ") fuzzy matches for ", spec.data[i, spec.name],
        ...
        if (length(specFuzzy) > 0) {
          ...
          if (verbose == TRUE) {
            message(paste("Fuzzy matches for ", spec.data[i,
                                                          spec.name], "were: ", paste(specFuzzy,
                                                                                      collapse = ", ")))
          }
          if (spec.data[i, "Fuzzy.two"] == TRUE && spec.data[i,
                                                             "Fuzzy.one"] == FALSE) {
            specFuzzy.2 <- NULL
            for (j in 1:length(specFuzzy)) {
              ...
              if (length(species.string3) < 3) {
                specFuzzy.2 <- c(specFuzzy.2, specFuzzy[j])
              }
            }
          ...
          if (Fuzzy.within == TRUE) {
            ...
            within.matches <- grepl(spec.data[i, spec.name],
                                    x = specFuzzy)
            specFuzzy <- specFuzzy[within.matches]
            if (verbose == T) {
              if (length(specFuzzy) > 0) {
                message(paste("Matches within for ",
                              spec.data[i, spec.name], "were: ",
                              paste(specFuzzy, collapse = ", ")))
              }
              else {
          ...
          for (j in 1:length(specFuzzy)) {
            WFO.match1 <- WFO.data[WFO.data$scientificName ==
                                     specFuzzy[j], ]
            if (j == 1) {
    ...
    if (fuzzy.matches == TRUE) {
      for (j in 1:nrow(WFO.match2)) {
        WFO.match2[j, "Fuzzy.dist"] <- as.numeric(utils::adist(WFO.match2[j,
                                                                          "scientificName"], y = spec.data[i, spec.name]))
        if (First.dist == TRUE) {
  ...
  if (acceptedNameUsageID.match == TRUE) {
    ...
    for (i in 1:nrow(WFO.out)) {
      ...
      if (is.null(WFO.out[i, "acceptedNameUsageID"]) ==
          TRUE) {
