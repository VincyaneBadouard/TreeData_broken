...
StatusCorrection2 <- function(
  ...
  if(DetectOnly %in% FALSE){
    ...
    for(c in InvariantColumns){
      if (!c %in% names(Data)){
        cc <- gsub("_TreeDataCor", "", c) # remove _TreeDataCor
        ...
        if (!cc %in% names(Data)){ # Col without - Cor exists?
          ...
          InvariantColumns[InvariantColumns == c] <- cc # If yes replace by the col name without cor
  ...
  Data <- Data[, StatusCorrectionByTree2(
    Plot = Plot,
    Year = Year,
    ...
    GenusCor = GenusCor,
    SpeciesCor = SpeciesCor,
    FamilyCor = FamilyCor,
    ScientificNameCor = ScientificNameCor,
...
#' LifeStatusCorrectionPlot(Rslt)
#'
StatusCorrectionByTree2 <- function(
    Plot,
    Year,
    LifeStatus,
    Diameter = NULL,
  ...
  # If no IdStem take IdTree
  if (is.null(IdStem) & is.null(IdTree)) {
    stop("You need to provide at least one of the following: IdStem, IdTree.")
  } else if((!"IdStem" %in% names(DataTree) | all(is.na(DataTree$IdStem))) &
