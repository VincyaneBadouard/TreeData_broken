...
- all(DataTree[, CensusYear] %in% PlotCensuses)
...
DataTree <- data.table(IdTree = "a", # 4 ind
                       CensusYear = seq(2000,2008, by = 2), # 9 DBH values each (2 years between 2 census)
...
order years of the tree and of the plot (PlotCensuses)
...
if(any(!is.na(LifeStatusCor) & DataTree$LifeStatusCor == 1)){ # already been alive tree
  ...
  RecruitYear <- min(DataTree$CensusYear[!is.na(DataTree$LifeStatusCor) & DataTree$LifeStatusCor == 1]) # recruitment year
  
  if(min(DataTree$CensusYear) > min(PlotCensuses) & sum(!is.na(DataTree$DBHCor)) > 0){ # if the 1st census of the plot is smaller than the 1st census of the tree, and there are measured DBH
